        ...
        fetch(`${process.env.REACT_APP_API_URL}/api/admin/editproduct/${id}`, {
            ...
            .then(res => {
                ...
                if (res.status === 200) {
                    // localStorage.setItem('auth-token', res.token)
                    // console.log(res)
                    setcheckbox({ ...checkbox, ...res.getProduct })

                    setformData({ ...formData, ...res.getProduct });
                            ...
                            <div className='card-body'>
                                <form onSubmit={(event) => submithandler(event)}>
                                    <div className='form-group mb-3'>
                                        ...
                                        <select name='category_id' value={formData.category_id == null ? '' : formData.category_id} onChange={(event) => changeHandler(event)}>
                                    ...
                                    <div className='form-group mb-3'>
                                        ...
                                        <input type='number' name='selling_price' value={formData.selling_price == null ? '' : formData.selling_price} onChange={(event) => changeHandler(event)} className='form-control' />
                                        <span>{formData.errorList["selling_price"]}</span>
                                    </div>
                                    <div className='form-group mb-3'>
                                        <label>Original price</label>
                                        <input type='number' name='original_price' value={formData.original_price == null ? '' : formData.original_price} onChange={(event) => changeHandler(event)} className='form-control' />
